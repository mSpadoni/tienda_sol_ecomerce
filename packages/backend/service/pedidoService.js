...
export default class pedidoService {
  ...
  constructor(repositorioPedido,repositorioUsuario,repositorioProducto) 
  {
    ...
    //TODO agregar repositorios usuario y producto
    ...
    pedidoData.items.forEach(item => {
      const producto = this.repositorioProducto.findById(item.productoId);  
        if (!producto || !producto.estaDisponible(item.cantidad)) {
            ...
            items.push({ producto, cantidad: item.cantidad,precioUnitario: producto.precio})
        }
    });
    
    const nuevoPedido = new Pedido(usuario,items, pedidoData.moneda, pedidoData.direccionEntrega, pedidoData.fecha);
  }
}