...
import {soloRol, validarToken} from "../middleware/autentificacionMiddlewares.js";
...
const pathPedidos = "/pedidos";
  ...
  router.get(pathPedidos+"/hechos",validarToken,soloRol(TipoUsuario.COMPRADOR), async (req, res, next) => {
    try {
  ...
  router.get(pathPedidos+"/recibidos",validarToken,soloRol(TipoUsuario.VENDEDOR), async (req, res, next) => {
    try {
      logger.http("Solicitud de pedidos del usuario id: " + req.params.id);
      await controler.findPedidosByID(req, res);
    } catch (err) {
